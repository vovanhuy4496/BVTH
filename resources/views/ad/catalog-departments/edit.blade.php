              ...
              <div class="col-sm-12 col-md-10 col-lg-8 col-xl-6">
                <div class="card">
                    <div class="card-header">
                      <i class="fa fa-align-justify"></i> {{ __('Chỉnh sửa') }}{{ $catalogDepartment->title ? ': '.$catalogDepartment->title : '' }}</div>
                    <div class="card-body">
                        <form method="POST" action="/catalog-departments/{{ $catalogDepartment->id }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="form-group row">
                                <label>Mô tả</label>
                                <textarea class="form-control" name="describe" required>{{ $catalogDepartment->describe }}</textarea>
                            </div>

                            <div class="form-group row summary-ckeditor">
                                <label>Nội dung</label>
                                <textarea class="form-control" id="summary-ckeditor" name="content" required>{{ $catalogDepartment->content }}</textarea>
                            </div>

                            <div class="form-group row">
                                <label class="margin-0-10-0-0 label-image">Hình ảnh hiện tại</label>
                                <img src="{{ URL::route('resizes', array('size' => 'thumbnail', 'imagePath' => 'BVTH/CatalogDepartments/'.$catalogDepartment->image_file_name)) }}" />
                            </div>
                            ...
                            <div class="form-group row bvth-status">
                                ...
                                <label style="width: 30px;" class="label-status margin-0-10-0-0">{{ $catalogDepartment->status == 1 ? 'Hiện' : 'Ẩn' }}</label>
                                <input type="checkbox"
                                        @if($catalogDepartment->status == 1)
                                            checked="checked"
                                        @endif
                            ...
                            <a href="{{ route('catalog-departments.index') }}" class="btn btn-block btn-primary">{{ __('Quay lại') }}</a> 
                        </form>
                    </div>
                </div>
              </div>
